    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for result in final_results:
                result_tasks.append(
                    asyncio.create_task(session.post("http://127.0.0.1:8082/filterUsernamesW", json=result)))
                result_tasks.append(
                    asyncio.create_task(session.post("http://127.0.0.1:8083/filterUsernamesD", json=result)))
